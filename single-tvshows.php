    ...
    <div class="container">
        ...
        <div class="single-details-grid">
            ...
            <div>
                ...
                <?php
                $dl_4k_url   = get_post_meta($post_id, 'download_url_4k', true) ?: ($vv_links['4k'] ?? "https://vidvault.ru/tv/{$clean_tmdb}/1/1?quality=2160p");
                $dl_1080_url = get_post_meta($post_id, 'download_url_1080p', true) ?: ($vv_links['1080p'] ?? "https://vidvault.ru/tv/{$clean_tmdb}/1/1?quality=1080p");
                $dl_720_url  = get_post_meta($post_id, 'download_url_720p', true) ?: ($vv_links['720p'] ?? "https://vidvault.ru/tv/{$clean_tmdb}/1/1?quality=720p");
                $dl_480_url  = get_post_meta($post_id, 'download_url_480p', true) ?: ($vv_links['480p'] ?? "https://vidvault.ru/tv/{$clean_tmdb}/1/1?quality=480p");
                ?>
    ...
    document.querySelectorAll('.btn-episode-select').forEach(function(btn) {
        btn.addEventListener('click', function() {
            ...
            if (hasDcEpisodes) {
                ...
                // Update multi-resolution download links for selected episode
                ['4k', '1080', '720', '480'].forEach(function(q) {
                    var btn = document.getElementById('dl-btn-' + q);
                    if (btn) {
                        var currUrl = btn.getAttribute('href') || '';
                        if (currUrl.indexOf('vidvault.ru') !== -1) {
                            var updatedUrl = currUrl.replace(/\/\d+\/\d+/, '/' + seasonNum + '/' + epNum);
